...
bool kvm_cpu_dirty_log_enabled(struct kvm *kvm)
{
...
set_pit2_out:
	...
	case KVM_CGM_GET_EPOCH_TOKEN: {
		struct kvm_cgm_data data;

		r = -ENOTTY;
		if (!kvm_x86_ops.cgm_get_epoch_token)
			goto out;
		r = -EFAULT;
		if (copy_from_user(&data, argp, sizeof(data)))
			goto out;
		data.size = 0;
		r = static_call(kvm_x86_cgm_get_epoch_token)(kvm, &data);
		if (r)
			goto out;
		r = -EFAULT;
		if (copy_to_user(argp, &data, sizeof(data)))
...
EXPORT_SYMBOL_GPL(kvm_complete_hypercall_exit);
