...
static int __tdx_reclaim_page(hpa_t pa, enum pg_level level)
{
	...
	if (WARN_ON_ONCE(err)) {
		pr_err("%s:%d:%s pa 0x%llx level %d\n",
		       __FILE__, __LINE__, __func__, pa, level);
...
void tdx_vm_free(struct kvm *kvm)
{
	...
	if (WARN_ON_ONCE(atomic64_read(&to_kvm_tdx(kvm)->ctl_pages) ||
			 tdx_nr_sept_pages(kvm, PG_LEVEL_512G) ||
			 tdx_nr_sept_pages(kvm, PG_LEVEL_1G) ||
			 tdx_nr_sept_pages(kvm, PG_LEVEL_2M) ||
			 tdx_nr_sept_pages(kvm, PG_LEVEL_4K) ||
...
static void tdx_vcpu_free_tdvpx(struct vcpu_tdx *tdx)
{
	...
	for (i = 0; i < tdx_info->nr_tdvpx_pages; i++) {
		...
		if (!td_page_pa)
			continue;
		if (tdx->initialized) {
			tdx_reclaim_control_page(td_page_pa);
...
void tdx_vcpu_reset(struct kvm_vcpu *vcpu, bool init_event)
{
	...
	if (KVM_BUG_ON(init_event, vcpu->kvm))
		return;
	if (KVM_BUG_ON(to_tdx(vcpu)->initialized, vcpu->kvm))
		return;
...
static noinstr void tdx_vcpu_enter_exit(struct vcpu_tdx *tdx)
{
	...
	if ((u16)tdx->exit_reason.basic == EXIT_REASON_EXCEPTION_NMI &&
	    /* tdexit_intr_info(vcpu) = tdx->exit_intr_info isn't set yet. */
	    is_nmi(kvm_r9_read(vcpu))) {
...
static int tdx_handle_ept_violation(struct kvm_vcpu *vcpu)
{
	...
	if (kvm_is_private_gpa(vcpu->kvm, tdexit_gpa(vcpu))) {
		 ...
		 * avoid COW allocations. For the RWX bits, always treat them
		 * as write faults. This will cause TDAUGPAGE failures due to
		 * aliasing a single HPA to multiple GPAs.
		 *
...
static int __tdx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t fastpath)
{
	...
	if (unlikely(exit_reason.full == (TDX_INCONSISTENT_MSR | MSR_IA32_TSX_CTRL))) {
		pr_err_once("TDX module is outdated. Upgrade the TDX module.\n");
		return 1;
	}
...
/* TODO: Remove this workaround once the TDX module is fixed. */ #if 1
...
/*
 * Workaround: TDH.SYS.RD(CPUID_CONFIG_VALUES + i) for some i returns 
wrong
...
static int tdx_migrate_from(struct kvm *dst, struct kvm *src)
{
	...
	for_each_memslot_pair(src_slots, dst_slots, src_memslot_iter, dst_memslot_iter) {
		...
		if (src_slot->flags & KVM_MEM_GUEST_MEMFD) {
			rcu_read_lock();
