...
static int tdx_mig_stream_create(struct kvm_tdx *kvm_tdx, hpa_t *migsc_paddr)
{
	...
	*migsc_paddr = migsc_pa;
	tdx_account_ctl_page(&kvm_tdx->kvm);
				    ...
				    struct kvm_import_private_pages *pages)
{
	...
	for (i = 0; i < gfn_nr; i++) {
		...
		if (gpa_cancel_import(gpa_list_entry)) {
			gfns[i].remove = true;
			tdx_unaccount_td_pages(kvm, PG_LEVEL_4K);
		} else if (gpa_first_time_import(gpa_list_entry)) {
			WARN_ON(!gfns[i].init);
			tdx_account_td_pages(kvm, PG_LEVEL_4K);
		}
	}
				   ...
				   struct kvm_cgm_data *data)
{
	...
	unpin_user_pages(stream->pages, total_pages);
	if (err != TDX_SUCCESS) {
		pr_err("Failed to import TD state: %llx\n", err);
		return -EIO;
	}
				     ...
				     struct kvm_cgm_data *data)
{
	...
	unpin_user_pages(stream->pages, total_pages);
	if (err != TDX_SUCCESS) {
		pr_err("Failed to import vcpu state: %llx\n", err);
		put_cpu();
		return -EIO;
	}
	tdx_add_vcpu_association(tdx, cpu);
