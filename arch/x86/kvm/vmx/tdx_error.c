...
void pr_tdx_error(u64 op, u64 error_code, const struct tdx_module_args *out)
{
	...
	if (!out) {
		pr_err_ratelimited("SEAMCALL %s(0x%016llx) failed: %s(0x%016llx)\n",
...
#define MSG_EPT_WALK_FAILED						\
	"SEAMCALL %s(0x%016llx) %s(0x%016llx) Secure EPT walk error: SEPTE 0x%016llx level_state 0x%llx %d %s\n"
		pr_err(MSG_EPT_WALK_FAILED,
		       tdx_seamcall_name(op), op, tdx_error_name(error_code), error_code,
		       entry.raw, level_state.raw, level_state.level,
		       tdx_spte_entry_state(level_state.state));
		break;
	}
	default:
#define MSG_FAILED	\
	"SEAMCALL %s(0x%016llx) failed: %s(0x%016llx) operand %s(0x%016llx) RCX 0x%016llx RDX 0x%016llx R8 0x%016llx R9 0x%016llx R10 0x%016llx R11 0x%016llx\n"
		pr_err_ratelimited(MSG_FAILED,
				   tdx_seamcall_name(op), op,
				   tdx_error_name(error_code), error_code,
