...
static __init int vt_hardware_setup(void)
{
	...
	if (enable_tdx) {
		...
		vt_x86_ops.write_unblock_private_page = tdx_write_unblock_private_page;
		vt_x86_ops.restore_private_page = tdx_restore_private_page;
	} else {
...
static int vt_cgm_get_epoch_token(struct kvm *kvm, struct kvm_cgm_data *data)
{
	if (!is_td(kvm))
		return -ENOTTY;

	return tdx_mig_get_epoch_token(kvm, data);
}

static int vt_cgm_set_epoch_token(struct kvm *kvm, struct kvm_cgm_data *data)
{
	if (!is_td(kvm))
		return -ENOTTY;
...
static int vt_cgm_get_vcpu_state(struct kvm_vcpu *vcpu, struct kvm_cgm_data *data)
{
	if (!is_td(vcpu->kvm))
		return -ENOTTY;
...
static int vt_cgm_end(struct kvm *kvm, long abort)
{
	if (!is_td(kvm))
		return -ENOTTY;

	return tdx_mig_end(kvm, abort);
}
