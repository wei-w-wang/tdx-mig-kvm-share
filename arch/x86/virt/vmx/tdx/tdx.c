...
u64 tdh_export_state_vp(u64 tdvpr, u64 mbmd_info, u64 page_list_info,
			u64 mig_stream_info, u64 *rdx)
{
	struct tdx_module_args args = {
		.rcx = tdvpr,
	...
	ret = seamcall_ret(TDH_EXPORT_STATE_VP, &args);
	*rdx = args.rdx;

	return ret;
}
EXPORT_SYMBOL_GPL(tdh_export_state_vp);

u64 tdh_import_state_vp(u64 tdvpr, u64 mbmd_info, u64 page_list_info,
			u64 mig_stream_info, u64 *rcx, u64 *rdx)
{
	struct tdx_module_args args = {
		.rcx = tdvpr,
	...
	ret = seamcall_ret(TDH_IMPORT_STATE_VP, &args);
	*rcx = args.rcx;
	*rdx = args.rdx;
